...
const DEFAULT_STAGING_MIN_AVAILABLE_BYTES = 100 * 1024 * 1024 * 1024;
...
interface DiskProofGate {
  ...
  blocks_preprocess_execution: boolean;
...
interface DiskProofSummary {
  ...
  preprocess_execution_blockers: string[];
}
...
export interface AdminDockerNasDiskProofReport {
  ...
  observations: {
    ...
    staging_min_available_bytes: number;
    ...
    staging_space_accepted: boolean;
    preprocess_space_accepted: boolean;
  };
...
function summarize(gates: DiskProofGate[]): DiskProofSummary {
  return {
    ...
    staging_execution_blockers: gates
      ...
      .map((item) => item.id),
    preprocess_execution_blockers: gates
      .filter((item) => item.blocks_preprocess_execution && item.status !== "pass")
      .map((item) => item.id)
  };
}
...
export function buildAdminDockerNasDiskProofReport(input: {
  ...
  const stagingMinAvailableBytes = asNumber(thresholds.staging_min_available_bytes) ?? DEFAULT_STAGING_MIN_AVAILABLE_BYTES;
  ...
  const stagingHasEnoughAbsoluteSpace = minAvailableBytes !== null && minAvailableBytes >= stagingMinAvailableBytes;
  const stagingSpaceAccepted = belowBlock || stagingHasEnoughAbsoluteSpace;
  const preprocessSpaceAccepted = belowBlock;
  const gates = [
    gate({
      ...
      blocks_preprocess_execution: false
    }),
    gate({
      ...
      blocks_preprocess_execution: !(input.disk_proof_json_path && raw),
    ...
    gate({
      ...
      blocks_preprocess_execution: Boolean(parsed.parse_error),
    ...
    gate({
      ...
      evidence: `attention=${attentionUsagePercent}, block=${blockUsagePercent}, staging_min_available_bytes=${stagingMinAvailableBytes}`,
      blocks_staging_execution: !thresholdsValid,
      blocks_preprocess_execution: !thresholdsValid,
      required_evidence: "Disk proof must set 0 < attention_usage_percent < block_usage_percent <= 100 and staging_min_available_bytes > 0 when provided."
    }),
    gate({
      ...
      blocks_preprocess_execution: expectedLibraryRoot !== EXPECTED_LIBRARY_ROOT,
    ...
    gate({
      ...
      blocks_preprocess_execution: !requiredChecksValid,
    ...
    gate({
      ...
      blocks_preprocess_execution: !checkValuesValid,
    ...
    gate({
      ...
      blocks_staging_execution: false,
      blocks_preprocess_execution: false,
      required_evidence: "Attention threshold is advisory for staging. Reduce usage below this level before sustained preprocessing."
    }),
    gate({
      id: "nas-disk-staging-space-available",
      title: "NAS disk has enough absolute free space for no-worker staging",
      category: "disk",
      status: stagingSpaceAccepted ? "pass" : "blocked",
      evidence: `max_usage_percent=${maxUsagePercent ?? "missing"}, min_available_bytes=${minAvailableBytes ?? "missing"}, staging_min_available_bytes=${stagingMinAvailableBytes}`,
      blocks_staging_execution: !stagingSpaceAccepted,
      blocks_preprocess_execution: false,
      required_evidence: "For initial Docker MVP staging with workers disabled, either stay below the block threshold or provide enough absolute free bytes for image pull and container startup."
    }),
    gate({
      id: "nas-disk-below-block-threshold",
      title: "NAS disk usage is below preprocess block threshold",
      category: "disk",
      status: belowBlock ? "pass" : "blocked",
      evidence: `max_usage_percent=${maxUsagePercent ?? "missing"}, block_usage_percent=${blockUsagePercent}`,
      blocks_staging_execution: false,
      blocks_preprocess_execution: !belowBlock,
      required_evidence: "Disk usage must be below the preprocess block threshold before enabling preprocessing or sustained write-heavy worker activity."
    }),
    gate({
  ...
  return {
    ...
    observations: {
      ...
      staging_min_available_bytes: stagingMinAvailableBytes,
      checks,
      ...
      staging_space_accepted: stagingSpaceAccepted,
      preprocess_space_accepted: preprocessSpaceAccepted
    },
    ...
    result: {
      ...
      summary: proofAccepted
        ? "NAS disk proof is accepted for initial no-worker Docker staging; Docker deploy and preprocessing are still governed by separate gates."
        : "NAS disk proof is blocked until sanitized df evidence shows enough safe staging space for /data/PublicLibrary."
    },
    artifacts: null
  };
}
...
export function toMarkdown(report: AdminDockerNasDiskProofReport): string {
  const lines = [
    ...
    `- Staging minimum available bytes: ${report.observations.staging_min_available_bytes}`,
    ...
    `- Staging space accepted: ${report.observations.staging_space_accepted ? "yes" : "no"}`,
    `- Preprocess space accepted: ${report.observations.preprocess_space_accepted ? "yes" : "no"}`,
    ...
    ...report.gates.map((item) => [
      ...
      item.blocks_preprocess_execution ? "yes" : "no",
      item.evidence,
