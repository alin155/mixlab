...
- 2026-06-29 UTC release-manager legacy rollback exception review：新增 `review:admin-docker-legacy-rollback-exception`，生成 `docs/acceptance/artifacts/admin-docker-legacy-rollback-exception-review-20260629T014448Z.json`，由 release-manager 角色接受一次性 legacy `latest` current/rollback 输入；该 artifact 明确 `release_execution_allowed=false`、`docker_deploy_allowed=false`。最新 release inputs `docs/acceptance/artifacts/admin-docker-release-inputs-20260629T014456Z.json` 已 `release_inputs_ready=true`，`current_image_tag=latest`、`rollback_image_tag=latest`、target `4cb5b18262e49894d4272b0fc940be6c1d2102b4`，但 `push_execution_allowed=false`。最新 NAS intake `docs/acceptance/artifacts/admin-docker-nas-release-inputs-intake-20260629T014505Z.json` 仍 blocked，剩余 `admin-worker-proof-accepted`、`nas-disk-proof-accepted`；最新 readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T014512Z.json` 为 `7/13` gate 通过、`release_review_ready=false`、`docker_upload_allowed=false`。本轮未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC NAS disk proof staging/preprocess split：`admin-docker-nas-disk-proof` 已把 no-worker Docker staging 余量和预处理余量分开判断。基于 UGOS returned evidence 中 `/data/PublicLibrary` 约 `98%` 使用率、约 `681GB` 可用空间，最新 disk proof `docs/acceptance/artifacts/admin-docker-nas-disk-proof-20260629T015141Z.json` 为 `proof_accepted=true`、`staging_space_accepted=true`、`preprocess_space_accepted=false`；这只清理 no-worker staging 的磁盘 carry-forward，不允许预处理或 Docker deploy。最新 intake `docs/acceptance/artifacts/admin-docker-nas-release-inputs-intake-20260629T015153Z.json` 仍 blocked，但剩余 blocker 已收敛为 `admin-worker-proof-accepted`；最新 readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T015249Z.json` 仍 `7/13` gate 通过、`release_review_ready=false`、`docker_upload_allowed=false`。同轮 Mac 侧 `/Volumes/MixLab` 未挂载，`docs/acceptance/artifacts/admin-docker-nas-access-preflight-20260629T014929Z.json` 仍通过 UGOS browserless path 保持 `ready-for-nas-collection`。本轮未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC Admin Docker push decision package：生成 `docs/acceptance/artifacts/admin-docker-push-decision-package-20260629T020021Z.json` 和 Markdown 同名报告，状态为 `ready-for-external-release-decision`；该 package 证明 release inputs 已 ready、target tag 匹配已 smoke 候选、no-worker staging disk proof accepted，且 staging execution blockers 只剩 `image-push-explicitly-approved`。报告内包含精确 `gh workflow run docker-admin.yml ... push_images=true ...` 命令供外部发布负责人审阅，但仍保持 `push_execution_allowed=false`、`docker_deploy_allowed=false`，不允许 NAS runtime 变更、image push、Docker deploy、worker 启用或预处理。
- 2026-06-29 UTC release readiness summary absorbed push decision package：最新 `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T020728Z.json` 已把 push-decision package 纳入总门禁，`push_decision_package_ready=true`，但 `image_push_allowed=false`、`docker_deploy_allowed=false` 继续保持；总计 `14` 个 gate、`8` 个通过、`6` 个阻塞，剩余阻断仍为 live readonly/parity、admin-worker proof、Cutter staged proof、NAS intake complete 和 staging runbook ready。本轮仅重算本地 artifact 和 validators，未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC GET-only live/parity/runbook/readiness refresh：只读探测 `http://192.168.1.27:18080` 生成 `docs/acceptance/artifacts/admin-docker-release-live-readonly-20260629T021039Z.json`，后续派生 `admin-docker-version-parity-plan-20260629T021049Z.json`、`admin-docker-staging-runbook-20260629T021109Z.json`、`admin-docker-release-readiness-summary-20260629T021117Z.json`。该轮继续确认 NAS `18080` 是旧 Admin API 合约：`auth/status`、`release-gates`、`data-loading/plan` 返回 `404`，`dashboard/metrics` 在 `10s` 超时；素材基线仍为 `/data/PublicLibrary`、`11394` total、`10471` ready、current index `v010471`，`library/status` 报告磁盘可用约 `16.6TB`。最新 summary 仍为 `14` 个 gate、`8` 个通过、`6` 个阻塞，`image_push_allowed=false`、`docker_deploy_allowed=false`；最新 runbook 显示 staging execution 层只剩 `image-push-explicitly-approved`，但完整 staging 仍被 parity、admin-worker proof 和 Cutter staged proof 阻断。本轮未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC worker remediation/intake/readiness refresh：`admin-worker-env-proof` 已新增无副作用 remediation plan，并基于旧 NAS returned evidence 生成 `docs/acceptance/artifacts/admin-worker-env-proof-20260629T021804Z.json`；当前 proof 仍 blocked，但目标值已明确为 `MIXLAB_ADMIN_DOCKER_MVP_MODE=v0.1`、`MIXLAB_ENABLE_LIBRARY_PREPROCESS_WORKER=0`、`MIXLAB_ENABLE_READY_PUBLISH_WORKER=0`、`MIXLAB_PREPROCESS_LIBRARY_ROOT=/data/PublicLibrary`。同轮重算 `admin-docker-nas-release-inputs-intake-20260629T021804Z.json`、`admin-docker-staging-runbook-20260629T021804Z.json`、`admin-docker-push-decision-package-20260629T021843Z.json`、`admin-docker-release-readiness-summary-20260629T021851Z.json`；总门禁仍 `14` 个 gate、`8` 个通过、`6` 个阻塞，`push_execution_allowed=false`、`docker_deploy_allowed=false`。本轮只改本地 validator/artifact/docs，未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC image-push-proof gate refresh：新增本地只读 `admin-docker-image-push-proof` gate，当前 proof `docs/acceptance/artifacts/admin-docker-image-push-proof-20260629T023443Z.json` 因 dry-run `push_images=false` 保持 blocked，确认不能把未推送镜像误判为可发布；同步重算 `docs/acceptance/artifacts/admin-docker-staging-runbook-20260629T023502Z.json`（`24` 个 gate、`20` 个通过、`4` 个阻塞，execution blocker 仍只剩 `image-push-explicitly-approved`）、`docs/acceptance/artifacts/admin-docker-push-decision-package-20260629T023513Z.json` 和 `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T023520Z.json`（仍 `14` 个 gate、`8` 个通过、`6` 个阻塞）。本轮只改本地 validator/artifact/docs，未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC worker remediation handoff review：`admin-worker-env-proof` 新增 `remediation_review` 与 `operator_handoff`，最新 `docs/acceptance/artifacts/admin-worker-env-proof-20260629T024459Z.json` 仍 `proof_accepted=false`，但 `remediation_review.status=accepted-for-runtime-owner-action`、`runtime_action_allowed=false`，把所需 NAS worker env 目标值和复验命令收口为可审评 checklist；`intake:admin-docker-nas-release-inputs` 也已把 image-push proof path 传入生成的 staging runbook。同步重算 `docs/acceptance/artifacts/admin-docker-nas-release-inputs-intake-20260629T024459Z.json`、`docs/acceptance/artifacts/admin-docker-staging-runbook-20260629T024459Z.json`、`docs/acceptance/artifacts/admin-docker-push-decision-package-20260629T024515Z.json`、`docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T024727Z.json`；总门禁仍 `14` 个 gate、`8` 个通过、`6` 个阻塞，`push_execution_allowed=false`、`docker_upload_allowed=false`。本轮只改本地 validator/artifact/docs，未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC NAS credentialed browserless readonly refresh：使用内存态 NAS 登录信息通过 UGOS/Docker 只读接口自动采集证据，UGOS preflight `docs/acceptance/artifacts/admin-docker-nas-ugos-api-preflight-20260629T025516Z.json` 为 `browserless-collection-ready`，returned evidence `docs/acceptance/artifacts/admin-docker-nas-ugos-returned-evidence-20260629T025704Z.json` 与目录 `docs/acceptance/artifacts/admin-docker-nas-ugos-returned-evidence-20260629T025704Z/admin-docker-release-inputs` 已生成；artifact 不记录密码、Cookie 或 token。当前 NAS Docker 仍是 `latest`，`admin-web`、`admin-api`、`admin-worker` 均运行；worker 仍为 `MIXLAB_ENABLE_LIBRARY_PREPROCESS_WORKER=1`、`MIXLAB_ENABLE_READY_PUBLISH_WORKER=1`，缺少 `MIXLAB_ADMIN_DOCKER_MVP_MODE` 与 `MIXLAB_PREPROCESS_LIBRARY_ROOT`。新 disk proof `docs/acceptance/artifacts/admin-docker-nas-disk-proof-20260629T025807Z.json` 显示 `/data/PublicLibrary` 使用率 `65%`、可用约 `16.6TB`，`staging_space_accepted=true`、`preprocess_space_accepted=true`；该结果只清理空间风险，不允许预处理或 Docker deploy。intake `docs/acceptance/artifacts/admin-docker-nas-release-inputs-intake-20260629T025807Z.json` 已 `release_inputs_ready=true`，current/rollback `latest`、target `4cb5b18262e49894d4272b0fc940be6c1d2102b4`，剩余 blocker 为 `admin-worker-proof-accepted`。GET-only live readonly `docs/acceptance/artifacts/admin-docker-release-live-readonly-20260629T025836Z.json` 仍证明 `18080` 是旧 Admin API 合约：`auth/status`、`release-gates`、`data-loading/plan` 返回 `404`，但 `library/status`、`dashboard/metrics`、`preprocess/supervisor/status` 可读，dashboard metrics 约 `5.9s`。parity `docs/acceptance/artifacts/admin-docker-version-parity-plan-20260629T025853Z.json` 与 readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T030404Z.json` 仍 blocked，总门禁 `8/14` 通过、`docker_upload_allowed=false`。本地 handoff kit 已恢复为 ready 并重新打包：`docs/acceptance/artifacts/admin-docker-nas-handoff-kit-20260629T030148Z.json`，archive sha256 `6e4bedf9c3fa4b4ff3ead949c41911c57112f3f321a5ef3b0e5c96c40d2fdb0a`；未传输到 NAS 共享目录。本轮未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC release-readiness next-action convergence：`admin-docker-release-readiness-summary` 已修正当前 next action 判定，`release_inputs_ready=true` 且 `returned_precheck_passed=true` 时不再建议重复运行 UGOS returned evidence collector；最新 readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T031242Z.json` 仍为 `14` 个 gate、`8` 个通过、`6` 个阻塞，`release_review_ready=false`、`docker_upload_allowed=false`，但下一步已准确收敛到旧 API/parity、admin-worker proof、Cutter staged proof、显式 push/release decision。该轮只改本地 release/acceptance tooling、artifact 和文档，未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC Admin Docker previous candidate refresh after branch drift：上一轮候选曾刷新为 `73ab7b355051d7325174111f955cd8356d42787d`，tag `admin-docker-candidate-73ab7b355051d7325174111f955cd8356d42787d`；GitHub dry-run `28346369749` 使用 `push_images=false` 成功，`Log in to GHCR` skipped，typecheck、searchd tests、delivery readiness、evidence kit、Docker MVP smoke、runtime/web image build、staging/readiness reports 均通过，未推送 GHCR 镜像。已归档 run artifact `docs/acceptance/artifacts/admin-docker-github-run-artifact-20260629T032141Z.json`、artifact readiness `docs/acceptance/artifacts/admin-docker-github-artifact-readiness-20260629T032141Z.json`、candidate-ref proof `docs/acceptance/artifacts/admin-docker-candidate-ref-proof-20260629T032206Z.json`、pre-staging handoff `docs/acceptance/artifacts/admin-docker-prestaging-handoff-20260629T032243Z.json`、release inputs `docs/acceptance/artifacts/admin-docker-release-inputs-20260629T032638Z.json`、Cutter staged proof plan `docs/acceptance/artifacts/admin-cutter-staged-proof-plan-20260629T032600Z.json`、image-push proof `docs/acceptance/artifacts/admin-docker-image-push-proof-20260629T032628Z.json`、push decision package `docs/acceptance/artifacts/admin-docker-push-decision-package-20260629T032621Z.json`、final readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T032649Z.json`。readiness 当时仍为 `14` 个 gate、`8` 个通过、`6` 个阻塞，`release_review_ready=false`、`docker_upload_allowed=false`；release inputs 已 ready，current/rollback 为 legacy `latest`，target 为 `73ab7b355051d7325174111f955cd8356d42787d`，但 intake 仍被 `admin-worker-proof-accepted` 阻断，image-push proof 因 dry-run 保持 blocked，push/deploy 均未批准。该轮只改本地 artifact/docs，未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC Admin Docker current candidate and NAS browserless readonly refresh：当前候选已刷新为 `9c015b9105e97954240020781f79daae3f954bde`，tag `admin-docker-candidate-9c015b9105e97954240020781f79daae3f954bde`；GitHub dry-run `28347083834` 使用 `push_images=false` 成功，`Log in to GHCR` skipped，typecheck、searchd tests、delivery readiness、evidence kit、Docker MVP smoke、runtime/web image build、staging/readiness reports 均通过，未推送 GHCR 镜像。已归档 run artifact `docs/acceptance/artifacts/admin-docker-github-run-artifact-20260629T034526Z.json`、artifact readiness `docs/acceptance/artifacts/admin-docker-github-artifact-readiness-20260629T034526Z.json`、candidate-ref proof `docs/acceptance/artifacts/admin-docker-candidate-ref-proof-20260629T034610Z.json`、pre-staging handoff `docs/acceptance/artifacts/admin-docker-prestaging-handoff-20260629T034627Z.json`、legacy rollback review `docs/acceptance/artifacts/admin-docker-legacy-rollback-exception-review-20260629T034655Z.json`、Cutter staged proof plan `docs/acceptance/artifacts/admin-cutter-staged-proof-plan-20260629T034732Z.json`。随后使用内存态 NAS 登录信息通过 UGOS/Docker 只读接口自动刷新证据，UGOS preflight `docs/acceptance/artifacts/admin-docker-nas-ugos-api-preflight-20260629T035403Z.json` 为 `browserless-collection-ready`，returned evidence `docs/acceptance/artifacts/admin-docker-nas-ugos-returned-evidence-20260629T035403Z.json` 与目录 `docs/acceptance/artifacts/admin-docker-nas-ugos-returned-evidence-20260629T035403Z/admin-docker-release-inputs` 已生成；artifact 不记录密码、Cookie 或 token。最新 release inputs `docs/acceptance/artifacts/admin-docker-release-inputs-20260629T035458Z.json` 已 ready，current/rollback 为 legacy `latest`，target 为 `9c015b9105e97954240020781f79daae3f954bde`；NAS disk proof `docs/acceptance/artifacts/admin-docker-nas-disk-proof-20260629T035458Z.json` 为 accepted，`/data/PublicLibrary` 使用率约 `65%`；worker proof `docs/acceptance/artifacts/admin-worker-env-proof-20260629T035458Z.json` 仍 blocked，当前 worker flags 仍为 `1/1` 且缺少目标 root/mode。最新 image-push proof `docs/acceptance/artifacts/admin-docker-image-push-proof-20260629T035556Z.json` 因 dry-run 保持 blocked；最新 staging runbook `docs/acceptance/artifacts/admin-docker-staging-runbook-20260629T035607Z.json` 为 `20/24` gate 通过，staging execution blocker 仍只剩 `image-push-explicitly-approved`，完整 staging 仍被 parity、worker proof、Cutter proof 阻断；push decision package `docs/acceptance/artifacts/admin-docker-push-decision-package-20260629T035614Z.json` 已准备但 `push_execution_allowed=false`。最终 readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T035624Z.json` 仍为 `14` 个 gate、`8` 个通过、`6` 个阻塞，`release_review_ready=false`、`docker_upload_allowed=false`。本轮未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC Admin Docker post-candidate docs/evidence boundary：`9c015b9105e97954240020781f79daae3f954bde..ae14db3abad0861d3d6df48f7ece137566a1a419` 只包含 `docs/architecture`、`docs/operations` 和 `docs/acceptance/artifacts` 证据文件，没有 `apps/`、`packages/`、`scripts/`、`docker/`、`deploy/`、`.github/workflows/docker-admin.yml`、`package.json`、`package-lock.json` 或 `tsconfig.json` 变化。按当前 Dockerfile，admin runtime image 不复制 `docs/`，admin web build 也不复制 `docs/`；因此 `ee672f1`、`c53941e`、`ae14db3` 不刷新 runtime candidate，release/staging target 仍保持 `9c015b9105e97954240020781f79daae3f954bde`。最新 proof：`docs/acceptance/artifacts/admin-docker-post-candidate-diff-proof-20260629T041352Z.json`；同轮 candidate scope audit：`docs/acceptance/artifacts/admin-docker-candidate-scope-20260629T041315Z.json`。后续如果触碰 Dockerfile 会复制的 build/runtime 输入，必须重新建 candidate tag 并重跑 `push_images=false` dry-run；纯证据/文档提交不能静默改 target image tag，也不能授权 image push、NAS runtime change、Docker deploy、worker 启用或预处理。
- 2026-06-29 UTC Admin Docker latest GET-only live/parity refresh：Mac 侧只读探测 `http://192.168.1.27:18080` 生成 `docs/acceptance/artifacts/admin-docker-release-live-readonly-20260629T041744Z.json`，派生 parity `docs/acceptance/artifacts/admin-docker-version-parity-plan-20260629T041744Z.json`、staging runbook `docs/acceptance/artifacts/admin-docker-staging-runbook-20260629T041828Z.json`、push decision package `docs/acceptance/artifacts/admin-docker-push-decision-package-20260629T041836Z.json` 和 readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T041847Z.json`。该轮仍确认 NAS 当前入口是旧 Admin API 合约：`/api/admin/auth/status`、`/api/admin/release-gates`、`/api/admin/data-loading/plan` 缺失；素材基线仍为 `11394` total、`10471` ready、current index `v010471`，磁盘 `65% healthy`，dashboard metrics 约 `6.1s`；总门禁仍为 `14` 个 gate、`8` 个通过、`6` 个阻塞，`release_review_ready=false`、`docker_upload_allowed=false`、`image_push_allowed=false`、`docker_deploy_allowed=false`。本轮未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- 2026-06-29 UTC Mac 本机 Admin 登录排障：本机 `3889` 配置为 `MIXLAB_ADMIN_LIBRARY_ROOT=/Volumes/MixLab/PublicLibrary`。当 `/Volumes/MixLab` 未挂载时，`auth/status` 会表现为 `has_admin=false`、登录失败，注册会因无法创建 `/Volumes/MixLab` 返回 `EACCES`；恢复 SMB `smb://192.168.1.27/MixLab` 挂载后，`/Volumes/MixLab/PublicLibrary` 存在且可写，管理端账号可登录，`library/status` 返回 ready `10471`、current index `v010471`。本轮只恢复 Mac SMB 挂载和本机 Admin Web 防崩修复，未启停 NAS 容器、未推送/拉取镜像、未写 NAS Docker runtime、未启动预处理。
- 2026-06-29 UTC Admin Docker previous MVP candidate refresh：候选 `be81398b3ade1b591122ee36a0a9566a889b1b2a` 曾作为上一轮 target，tag `admin-docker-candidate-be81398b3ade1b591122ee36a0a9566a889b1b2a` 已推送到 origin；GitHub Actions run `28361098922` 使用 `push_images=false` 成功，未推送 GHCR 镜像。该候选已被后续 `8a7939c1e8b2220d45d52c5fe329692516c32e4d` 覆盖。
- 2026-06-29 UTC Admin Docker previous health-gate candidate refresh：候选 `8a7939c1e8b2220d45d52c5fe329692516c32e4d` 曾作为 target，tag `admin-docker-candidate-8a7939c1e8b2220d45d52c5fe329692516c32e4d` 已推送到 origin。GitHub Actions run `28362489637` 使用 `push_images=false` 成功，`Log in to GHCR` skipped，typecheck、searchd tests、evidence kit、Docker MVP smoke、runtime/web image build、release gate artifact 上传均通过；candidate-ref proof `docs/acceptance/artifacts/admin-docker-candidate-ref-proof-20260629T093721Z.json` accepted。该候选随后被 `5a50922bc82f1b6728f247ed33e5885ab8cf6bef` 覆盖，原因是上线前只读探测发现 admin-web 公网入口 `/health` 返回前端 HTML，需补齐 nginx health 代理。
- 2026-06-29 UTC Admin Docker current MVP candidate refresh：当前候选为 `5a50922bc82f1b6728f247ed33e5885ab8cf6bef`，tag `admin-docker-candidate-5a50922bc82f1b6728f247ed33e5885ab8cf6bef` 已推送到 origin。GitHub Actions run `28363633436` 使用 `push_images=false` 成功，`Log in to GHCR` skipped，typecheck、searchd tests、evidence kit、Docker MVP smoke、runtime/web image build、release gate artifact 上传均通过；candidate-ref proof `docs/acceptance/artifacts/admin-docker-candidate-ref-proof-20260629T095825Z.json` accepted。当前 release inputs 使用 current/rollback `9c015b9105e97954240020781f79daae3f954bde`、target `5a50922bc82f1b6728f247ed33e5885ab8cf6bef`；latest readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T100114Z.json` 为 `13/14` pass，唯一 blocker 是显式 `push_images=true` release-owner approval。最新 release-owner runbook `docs/acceptance/artifacts/admin-docker-release-owner-runbook-20260629T100121Z.json` 为 `ready-for-release-owner-review` 且 `runbook_blockers=[]`，但 `push_execution_allowed=false`、`docker_deploy_allowed=false`、`preprocess_execution_allowed=false`、`mvp_completion_allowed=false` 仍保持关闭。未执行 `push_images=true`，未启停 NAS 容器，未推送/拉取镜像，未写 NAS runtime，未写 `PublicLibrary`，未启动预处理。
