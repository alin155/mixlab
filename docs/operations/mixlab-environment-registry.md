...
- 2026-06-29 UTC Admin Docker latest GET-only live/parity refresh：Mac 侧只读探测 `http://192.168.1.27:18080` 生成 `docs/acceptance/artifacts/admin-docker-release-live-readonly-20260629T040824Z.json`，派生 parity `docs/acceptance/artifacts/admin-docker-version-parity-plan-20260629T040825Z.json` 和 readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T040950Z.json`。该轮仍确认 NAS 当前入口是旧 Admin API 合约：`/api/admin/auth/status`、`/api/admin/release-gates`、`/api/admin/data-loading/plan` 缺失；总门禁仍为 `14` 个 gate、`8` 个通过、`6` 个阻塞，`release_review_ready=false`、`docker_upload_allowed=false`、`image_push_allowed=false`、`docker_deploy_allowed=false`。本轮未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
