...
- 2026-06-28 UTC Admin Docker workflow tag hardening：`.github/workflows/docker-admin.yml` 已改为只推送 `ghcr.io/alin155/mixlab-admin-runtime:${{ github.sha }}` 与 `ghcr.io/alin155/mixlab-admin-web:${{ github.sha }}`，不再推送 mutable `latest`；`audit:delivery-readiness` 和目标证据测试会拒绝重新引入 `latest` tag。该变更会让先前 `f9aa9bc7dea187dc3029c90389d5c3c08938dd58` Docker dry-run 候选证据过期，后续需要刷新 GitHub candidate dry-run 后再进入 NAS staging。
