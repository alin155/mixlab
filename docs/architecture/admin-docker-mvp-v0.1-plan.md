...
- current worker remediation proof：`docs/acceptance/artifacts/admin-worker-env-proof-20260629T021804Z.json`，`proof_accepted=false`，但已生成无副作用 remediation plan；目标值为 `MIXLAB_ADMIN_DOCKER_MVP_MODE=v0.1`、`MIXLAB_ENABLE_LIBRARY_PREPROCESS_WORKER=0`、`MIXLAB_ENABLE_READY_PUBLISH_WORKER=0`、`MIXLAB_PREPROCESS_LIBRARY_ROOT=/data/PublicLibrary`。该 artifact 只读取脱敏 returned evidence，不批准 NAS `.env` 编辑、容器重启、worker 启用或预处理。
