...
2026-06-28 已完成当前 HEAD 的 GitHub Docker-capable dry-run 证据收口，但仍不代表 NAS 已发布，也不代表可以上传镜像。历史候选 `b062bc387c1fdb2a391320c1c36233b782cb000a`、`97f2d513a4a27315929b4d964320d4170b4b4631` 只保留为旧证据；当前候选已刷新为 `f9aa9bc7dea187dc3029c90389d5c3c08938dd58`。

- current candidate tag：`admin-docker-candidate-f9aa9bc7dea187dc3029c90389d5c3c08938dd58`，远端 tag 指向 `f9aa9bc7dea187dc3029c90389d5c3c08938dd58`。
- current tag dry-run：GitHub Actions run `28333837849`，ref `admin-docker-candidate-f9aa9bc7dea187dc3029c90389d5c3c08938dd58`，`push_images=false`，workflow success；typecheck、searchd tests、evidence tooling、Docker MVP smoke、runtime/web image build、release-gate reports 全部通过，未推送 GHCR 镜像。
- current tag dry-run 证据：`docs/acceptance/artifacts/admin-docker-github-run-artifact-20260628T194849Z.json`，`current_worktree_candidate_ready=true`，对应 artifact readiness `docs/acceptance/artifacts/admin-docker-github-artifact-readiness-20260628T194849Z.json` 为 `candidate-ready`，`docker_deploy_allowed=false`。
- current candidate-ref proof：`docs/acceptance/artifacts/admin-docker-candidate-ref-proof-20260628T194955Z.json`，`candidate_ref_proof_accepted=true`，`docker_deploy_allowed=false`。
- current pre-staging handoff：`docs/acceptance/artifacts/admin-docker-prestaging-handoff-20260628T195010Z.json`，`ready_to_request_release_inputs=true`，但 `staging_execution_ready=false`；staging 仍被 NAS disk risk、显式 push approval、current/rollback tag、staged live-readonly、admin-worker env proof、Cutter compatibility proof 阻断。
- current release inputs：`docs/acceptance/artifacts/admin-docker-release-inputs-20260628T195028Z.json`，因缺 NAS image proof、current/rollback tag 仍 `blocked`，`push_execution_allowed=false`。
- current NAS release-inputs handoff：`docs/acceptance/artifacts/admin-docker-nas-release-inputs-handoff-20260628T195036Z.json`，`handoff_package_ready=true`，bundle 指向 `f9aa9bc7dea187dc3029c90389d5c3c08938dd58`，包含 `OPERATOR-CHECKLIST.md`，本地 `validate-returned-evidence.sh` 会在 returned evidence precheck 和 intake 后自动刷新 release readiness summary，但不允许 push/deploy。
- current NAS handoff portable kit：`docs/acceptance/artifacts/admin-docker-nas-handoff-kit-20260628T195043Z.json`，`kit_ready=true`，便携目录为 `dist/acceptance/admin-docker-nas-handoff-kit`，单文件包为 `dist/acceptance/admin-docker-nas-handoff-kit.tar.gz`，sha256 `8bcc72b402dabbcc055a58bc0e1c3e7841035e8eeed8eb4af904aa1733f05689`；包含 `KIT-SELF-CHECK.sh` 与 `KIT-FILES.sha256`，可复制到 NAS desktop/NAS shell host 用于只读收集 `admin-docker-release-inputs/` 返回证据。
- current NAS handoff transfer：`docs/acceptance/artifacts/admin-docker-nas-handoff-transfer-20260628T195043Z.json`，已将 `admin-docker-nas-handoff-kit.tar.gz` 复制到 `/Volumes/MixLab/安装包/mixlab-admin-docker-handoff/`，目标文件 sha256 同为 `8bcc72b402dabbcc055a58bc0e1c3e7841035e8eeed8eb4af904aa1733f05689`；本步骤只写 NAS `安装包` 交付目录，不接触 Docker runtime、不写 `PublicLibrary`、不启停容器、不运行预处理。
...
- workflow tag hardening：`.github/workflows/docker-admin.yml` 已改为只给 `mixlab-admin-runtime` 与 `mixlab-admin-web` 推送 immutable `${{ github.sha }}` tag，不再推送 mutable `latest`。`latest` 仍可被旧 NAS 栈作为当前观察事实拒绝，但新候选发布流程不应继续覆盖 `latest`；因此旧 dry-run 候选 `f9aa9bc7dea187dc3029c90389d5c3c08938dd58` 的 workflow 证据在本次改动后需要重新刷新。
