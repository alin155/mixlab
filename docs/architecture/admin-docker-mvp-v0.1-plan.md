...
| NAS access preflight | SSH/DSM/Docker TCP/`8080` 关闭；`18080`/`9999` 打开；SMB `/Volumes/MixLab` 已挂载，且可见 handoff archive `/Volumes/MixLab/安装包/mixlab-admin-docker-handoff/admin-docker-nas-handoff-kit.tar.gz`；但未发现 compose/.env 或 returned evidence；最新证据 `docs/acceptance/artifacts/admin-docker-nas-access-preflight-20260628T204410Z.json` | Handoff 已传到 NAS 共享，但当前仍不能直接从 Mac 收集 NAS Docker release inputs。 |
| NAS UGOS API preflight | `http://192.168.1.27:9999/desktop/` 可读，UGOS desktop `1.15.0.77682` / build `4/23/2026`；JSON API 可达，但当前无认证会话，Docker app context 与 container list 不可读；证据 `docs/acceptance/artifacts/admin-docker-nas-ugos-api-preflight-20260628T191949Z.json` | UGOS API 暂不能替代 SSH/桌面 handoff；如果后续提供临时 authenticated Cookie/X-Ugreen-Auth，只能通过环境变量传入，不落盘。 |
| NAS release inputs | 未发现由 NAS collector 返回的正式 `admin-docker-release-inputs/`；NAS 桌面只读观察已确认旧容器和 worker flags，且桌面只读证据转换包 `docs/acceptance/artifacts/admin-docker-nas-desktop-returned-evidence-20260628T200756Z/admin-docker-release-inputs` 已被 intake 阻断 | 桌面转换包只用于诊断旧栈阻断原因，不能替代正式 NAS collector 返回包；必须通过 NAS desktop/本机 shell/临时 SSH 跑 collector 并把返回包带回 Mac 后才能继续 release review。 |
...
- current candidate tag：`admin-docker-candidate-25fe2264de7b391a56e770a8acb6bf40ebec3863`，远端 tag 指向 `25fe2264de7b391a56e770a8acb6bf40ebec3863`。
...
- tag dry-run：GitHub Actions run `28335143445`，ref `admin-docker-candidate-25fe2264de7b391a56e770a8acb6bf40ebec3863`，`push_images=false`，workflow success；typecheck、searchd tests、delivery readiness、evidence kit、Docker MVP smoke、runtime/web image build、staging/readiness reports 全部通过，未推送 GHCR 镜像。
- current tag dry-run 证据：`docs/acceptance/artifacts/admin-docker-github-run-artifact-20260628T203848Z.json`，对应 artifact readiness `docs/acceptance/artifacts/admin-docker-github-artifact-readiness-20260628T203848Z.json` 为 `candidate-ready`，image tag/build SHA 均为 `25fe2264de7b391a56e770a8acb6bf40ebec3863`，`docker_deploy_allowed=false`。
- current candidate-ref proof：`docs/acceptance/artifacts/admin-docker-candidate-ref-proof-20260628T203900Z.json`，`candidate_ref_proof_accepted=true`，`docker_deploy_allowed=false`。
- current pre-staging handoff：`docs/acceptance/artifacts/admin-docker-prestaging-handoff-20260628T203935Z.json`，`ready_to_request_release_inputs=true`，但 `staging_execution_ready=false`；staging execution 前置阻断为 `nas-disk-risk-carried-forward`、`explicit-push-approval-required`、`current-and-rollback-tags-required`。
- current release inputs：`docs/acceptance/artifacts/admin-docker-release-inputs-20260628T203944Z.json`，仍 `blocked`，`push_execution_allowed=false`，`docker_deploy_allowed=false`；当前阻断为 NAS image proof 未接受、current/rollback/target tag 未完整、workflow command 占位未替换。
- current NAS release-inputs handoff：`docs/acceptance/artifacts/admin-docker-nas-release-inputs-handoff-20260628T204343Z.json`，`handoff_package_ready=true`，bundle 指向 `25fe2264de7b391a56e770a8acb6bf40ebec3863`；本地 `validate-returned-evidence.sh` 会在 returned evidence precheck 和 intake 后自动刷新 release readiness summary，但不允许 push/deploy。
- current NAS handoff portable kit：`docs/acceptance/artifacts/admin-docker-nas-handoff-kit-20260628T204403Z.json`，`kit_ready=true`，便携目录为 `dist/acceptance/admin-docker-nas-handoff-kit`，单文件包为 `dist/acceptance/admin-docker-nas-handoff-kit.tar.gz`，sha256 `6a3474ca67b3a4d8c09210ff3a7e8441053faead3b5a8577dce345e75058f132`；包含 `KIT-SELF-CHECK.sh` 与 `KIT-FILES.sha256`。
- current NAS handoff transfer：`docs/acceptance/artifacts/admin-docker-nas-handoff-transfer-20260628T204403Z.json`，已将 `admin-docker-nas-handoff-kit.tar.gz` 复制到 `/Volumes/MixLab/安装包/mixlab-admin-docker-handoff/`，目标文件 sha256 同为 `6a3474ca67b3a4d8c09210ff3a7e8441053faead3b5a8577dce345e75058f132`；本步骤只写 NAS `安装包` 交付目录，不接触 Docker runtime、不写 `PublicLibrary`、不启停容器、不运行预处理。
- NAS desktop authenticated readonly follow-up：通过 NAS 桌面 Docker UI 只读确认项目 `mixlab-server` 运行中，`admin-web`、`admin-api`、`admin-worker` 三个容器均运行且仍使用 mutable `latest`；`admin-web` 暴露 `18080 -> 80/TCP`，`admin-api` 与 `admin-worker` 都读写挂载 `/data/PublicLibrary`，旧 worker flags 仍开启。本轮未启停容器、未推送/拉取镜像、未写 NAS runtime、未写 `PublicLibrary`、未启动预处理。
- live readonly refresh：`docs/acceptance/artifacts/admin-docker-release-live-readonly-20260628T203916Z.json`，GET-only 探测 NAS `http://192.168.1.27:18080`；Admin Web root 可达，但 `/api/admin/auth/status`、`/api/admin/release-gates`、`/api/admin/data-loading/plan` 仍缺失，说明 NAS 仍是旧 Admin API 合约；素材统计保持 ready `10471`、current index `v010471`、total `11394`，queued `904`、index-required `19`。
