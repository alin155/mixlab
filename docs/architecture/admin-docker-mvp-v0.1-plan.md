...
- current candidate tag：`admin-docker-candidate-b945418df2a447fd39bb9c88f781322594fc11c0`，远端 tag 指向 `b945418df2a447fd39bb9c88f781322594fc11c0`。
...
- current candidate-ref proof：`docs/acceptance/artifacts/admin-docker-candidate-ref-proof-20260628T224835Z.json`，`candidate_ref_proof_accepted=true`，`docker_deploy_allowed=false`。
- current pre-staging handoff：`docs/acceptance/artifacts/admin-docker-prestaging-handoff-20260628T224856Z.json`，`ready_to_request_release_inputs=true`，但 `staging_execution_ready=false`；staging execution 前置阻断为 `nas-disk-risk-carried-forward`、`explicit-push-approval-required`、`current-and-rollback-tags-required`。
- current release inputs：`docs/acceptance/artifacts/admin-docker-release-inputs-20260628T224937Z.json`，仍 `blocked`，`push_execution_allowed=false`，`docker_deploy_allowed=false`；当前阻断为 NAS image proof 未接受、current/rollback/target tag 未完整、workflow command 占位未替换。
- current NAS release-inputs handoff：`docs/acceptance/artifacts/admin-docker-nas-release-inputs-handoff-20260628T224856Z.json`，`handoff_package_ready=true`，bundle 指向 `b945418df2a447fd39bb9c88f781322594fc11c0`；本地 `validate-returned-evidence.sh` 会在 returned evidence precheck 和 intake 后自动刷新 release readiness summary，但不允许 push/deploy。
- current NAS handoff portable kit：`docs/acceptance/artifacts/admin-docker-nas-handoff-kit-20260628T224903Z.json`，`kit_ready=true`，便携目录为 `dist/acceptance/admin-docker-nas-handoff-kit`，单文件包为 `dist/acceptance/admin-docker-nas-handoff-kit.tar.gz`，sha256 `bd135af00c499a933a06a4ede5bf4ef6b8fbe605ebd04b906b0b86d4e01334a6`；包含 `KIT-SELF-CHECK.sh` 与 `KIT-FILES.sha256`。
- current NAS handoff transfer：`docs/acceptance/artifacts/admin-docker-nas-handoff-transfer-20260628T224903Z.json`，已将 `admin-docker-nas-handoff-kit.tar.gz` 复制到 `/Volumes/MixLab/安装包/mixlab-admin-docker-handoff/`，目标文件 sha256 同为 `bd135af00c499a933a06a4ede5bf4ef6b8fbe605ebd04b906b0b86d4e01334a6`；本步骤只写 NAS `安装包` 交付目录，不接触 Docker runtime、不写 `PublicLibrary`、不启停容器、不运行预处理。
