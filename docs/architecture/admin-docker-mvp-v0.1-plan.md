...
| NAS release inputs | Mac 侧已通过 UGOS browserless 只读采集生成 sanitized returned evidence：`docs/acceptance/artifacts/admin-docker-nas-ugos-returned-evidence-20260629T025704Z/admin-docker-release-inputs`。该证据显示当前旧栈仍为 `latest` 镜像、worker flags 为 `1/1`、缺少 `MIXLAB_ADMIN_DOCKER_MVP_MODE` 与 `MIXLAB_PREPROCESS_LIBRARY_ROOT`，但磁盘已更新为 `65%` healthy。当前 candidate 已刷新为 `73ab7b355051d7325174111f955cd8356d42787d`；最新 release inputs `docs/acceptance/artifacts/admin-docker-release-inputs-20260629T032638Z.json` 已生成 current/rollback `latest`、target `73ab7b355051d7325174111f955cd8356d42787d`。最新 disk proof `docs/acceptance/artifacts/admin-docker-nas-disk-proof-20260629T032638Z.json` 为 `proof_accepted=true`、`staging_space_accepted=true`、`preprocess_space_accepted=true`；最新 intake `docs/acceptance/artifacts/admin-docker-nas-release-inputs-intake-20260629T032638Z.json` 已使 `release_inputs_ready=true`，但仍因 `admin-worker-proof-accepted` blocked；最新 readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T032649Z.json` 仍为 `8/14` gate 通过、`docker_upload_allowed=false`。 | UGOS returned evidence 只用于只读诊断和本地门禁输入，不等于正式发布批准；legacy latest rollback exception 只能用于 release-input 生成，不能绕过 worker proof、Cutter staged proof、显式 push approval 和最终 release review。磁盘 proof 只清理空间风险，不开放预处理；预处理仍受 worker flags、live release gates、路径和 supervisor 门禁约束。 |
...
| 磁盘风险 | 最新 UGOS returned evidence 和 live-readonly 均显示 `/data/PublicLibrary` 约 `65%` 使用率、约 `16.6TB` 可用；`docs/acceptance/artifacts/admin-docker-nas-disk-proof-20260629T032638Z.json` 已同时接受 staging 和 preprocess space。 | 空间风险已从当前阻断中移除，但这不等于允许预处理；worker flags、旧 API contract、release gates、Cutter staged proof 和显式发布决策仍阻断上线/预处理。 |

## 当前已完成远端候选证据

2026-06-28 已完成 Docker runtime candidate 的 GitHub Docker-capable dry-run 证据收口，2026-06-29 UTC 又因 branch drift boundary 重新对当前 HEAD 做 `push_images=false` dry-run。当前 Docker runtime candidate 已刷新为 `73ab7b355051d7325174111f955cd8356d42787d`，但仍不代表 NAS 已发布，也不代表可以上传镜像。历史候选 `b062bc387c1fdb2a391320c1c36233b782cb000a`、`97f2d513a4a27315929b4d964320d4170b4b4631`、`f9aa9bc7dea187dc3029c90389d5c3c08938dd58`、`25fe2264de7b391a56e770a8acb6bf40ebec3863`、`b945418df2a447fd39bb9c88f781322594fc11c0`、`4cb5b18262e49894d4272b0fc940be6c1d2102b4` 只保留为旧证据。
...
- current candidate tag：`admin-docker-candidate-73ab7b355051d7325174111f955cd8356d42787d`，远端 tag 指向 `73ab7b355051d7325174111f955cd8356d42787d`。
- current tag dry-run：GitHub Actions run `28346369749`，ref `admin-docker-candidate-73ab7b355051d7325174111f955cd8356d42787d`，`push_images=false`，workflow success；typecheck、searchd tests、delivery readiness、evidence kit、Docker MVP smoke、runtime/web image build、staging/readiness reports 全部通过；`Log in to GHCR` skipped，未推送 GHCR 镜像。该 run 有 GitHub Actions Node.js 20 deprecation annotation，但不是当前候选阻断。
- current tag dry-run 证据：`docs/acceptance/artifacts/admin-docker-github-run-artifact-20260629T032141Z.json`，对应 artifact readiness `docs/acceptance/artifacts/admin-docker-github-artifact-readiness-20260629T032141Z.json` 为 `candidate-ready`，image tag/build SHA 均为 `73ab7b355051d7325174111f955cd8356d42787d`，`docker_deploy_allowed=false`。
- current candidate-ref proof：`docs/acceptance/artifacts/admin-docker-candidate-ref-proof-20260629T032206Z.json`，`candidate_ref_proof_accepted=true`，远端 tag、GitHub run head 和 expected SHA 均为 `73ab7b355051d7325174111f955cd8356d42787d`，`docker_deploy_allowed=false`。
- post-candidate branch drift boundary：`4cb5b18262e49894d4272b0fc940be6c1d2102b4` 之后的 branch drift 已通过 `73ab7b355051d7325174111f955cd8356d42787d` tag dry-run 重新收口。由于 `docker/admin-runtime.Dockerfile` 会 `COPY scripts ./scripts`，后续任何影响 runtime image 内容的新提交仍不能自动继承本轮 smoke 证据，必须重新创建候选 tag 并完成 `push_images=false` dry-run / smoke。
- current pre-staging handoff：`docs/acceptance/artifacts/admin-docker-prestaging-handoff-20260629T032243Z.json`，`ready_to_request_release_inputs=true`，但 `staging_execution_ready=false`；staging execution 前置阻断仍为 `nas-disk-risk-carried-forward`、`explicit-push-approval-required`、`current-and-rollback-tags-required`。
- current release-manager exception review：`docs/acceptance/artifacts/admin-docker-legacy-rollback-exception-review-20260629T032343Z.json`，`exception_review_accepted=true`，确认当前旧 NAS `latest` 只能作为一次性 current/rollback release-input 输入；该 artifact 明确 `release_execution_allowed=false`、`docker_deploy_allowed=false`，不能替代 push/deploy/release approval。
- current release inputs：`docs/acceptance/artifacts/admin-docker-release-inputs-20260629T032638Z.json`，`ready-for-release-decision`，`release_inputs_ready=true`，`push_execution_allowed=false`，`docker_deploy_allowed=false`；已读取当前 pre-staging、candidate-ref、legacy rollback plan 与 release-manager review artifact，生成 `current_image_tag=latest`、`rollback_image_tag=latest`、`target_image_tag=73ab7b355051d7325174111f955cd8356d42787d`，当前 release-input 阻断清零，但 push 仍被 `explicit-release-approval-required` 阻断。
- current NAS release-inputs handoff：当前主路径已转为 UGOS browserless returned evidence direct intake；`admin-docker-nas-release-inputs-handoff-latest` 仍是旧的便携 handoff 包，不应作为 `73ab7b355051d7325174111f955cd8356d42787d` 的新候选 handoff 证据。若后续重新走离线 handoff 路径，需要在 NAS 证据尚未 intake 前单独重建 handoff kit；本轮未传输新 handoff 到 NAS。
...
- 当前 plan artifact `docs/acceptance/artifacts/admin-cutter-staged-proof-plan-20260629T032600Z.json` 已准备好候选 `73ab7b355051d7325174111f955cd8356d42787d` 的 `windows_acceptance`、`real_cut_smoke`、可选 `desktop_ui_screenshot_smoke` 命令模板，并把最终验证收口到 `npm run validate:admin-cutter-compatibility-proof`。
- `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T032649Z.json` 已把该 staged plan 记入 sources/observations；这只清理“证明步骤是否准备好”的追踪缺口，不清理最终 `cutter-proof-accepted` 门禁。
