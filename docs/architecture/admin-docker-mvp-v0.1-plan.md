...
- current release readiness summary：`docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T030404Z.json`，`release_review_ready=false`，`docker_upload_allowed=false`；最新汇总仍为 `14` 个 gate、`8` 个通过、`6` 个阻塞，且 observations 已记录 `release_inputs_ready=true`、`worker_remediation_review_status=accepted-for-runtime-owner-action`、`worker_remediation_runtime_action_allowed=false`；`automation_boundary` 明确本地只读/证据工作可继续，但 NAS runtime 变更、image push、Docker deploy 均不允许。
