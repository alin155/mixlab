...
- release readiness summary：`docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260628T111927Z.json`，`release_review_ready=false`，`docker_upload_allowed=false`；已纳入 GitHub candidate artifact、NAS access preflight、handoff kit 与最新 staging runbook 证据，其中 Docker candidate smoke、`nas-collection-path-prepared` 和 `nas-handoff-kit-ready` 已通过，但仍有 `8` 个 release review blockers。
