...
| NAS release inputs | Mac 侧已通过 UGOS browserless 只读采集生成最新 sanitized returned evidence：`docs/acceptance/artifacts/admin-docker-nas-ugos-returned-evidence-20260629T025704Z/admin-docker-release-inputs`。该证据显示当前旧栈仍为 `latest` 镜像、worker flags 为 `1/1`、缺少 `MIXLAB_ADMIN_DOCKER_MVP_MODE` 与 `MIXLAB_PREPROCESS_LIBRARY_ROOT`，但磁盘已更新为 `65%` healthy。最新 disk proof `docs/acceptance/artifacts/admin-docker-nas-disk-proof-20260629T025807Z.json` 为 `proof_accepted=true`、`staging_space_accepted=true`、`preprocess_space_accepted=true`；最新 intake `docs/acceptance/artifacts/admin-docker-nas-release-inputs-intake-20260629T025807Z.json` 已使 `release_inputs_ready=true`，但仍因 `admin-worker-proof-accepted` blocked；最新 readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T031242Z.json` 仍为 `8/14` gate 通过、`docker_upload_allowed=false`。 | UGOS returned evidence 只用于只读诊断和本地门禁输入，不等于正式发布批准；legacy latest rollback exception 只能用于 release-input 生成，不能绕过 worker proof、Cutter staged proof、显式 push approval 和最终 release review。磁盘 proof 只清理空间风险，不开放预处理；预处理仍受 worker flags、live release gates、路径和 supervisor 门禁约束。 |
...
| 磁盘风险 | 最新 UGOS returned evidence 和 live-readonly 均显示 `/data/PublicLibrary` 约 `65%` 使用率、约 `16.6TB` 可用；`docs/acceptance/artifacts/admin-docker-nas-disk-proof-20260629T025807Z.json` 已同时接受 staging 和 preprocess space。 | 空间风险已从当前阻断中移除，但这不等于允许预处理；worker flags、旧 API contract、release gates、Cutter staged proof 和显式发布决策仍阻断上线/预处理。 |
