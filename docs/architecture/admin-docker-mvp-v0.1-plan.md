...
| NAS UGOS API preflight | `http://192.168.1.27:9999/desktop/` 可读，UGOS desktop `1.15.0.77682` / build `4/23/2026`；使用临时已登录桌面会话的 query token 可只读访问 session/current-user/Docker app uid/share/sysinfo，但静态 `ContainerList` 端点仍返回 `9405`，Docker UI 实际 container list 走加密 view API；证据 `docs/acceptance/artifacts/admin-docker-nas-ugos-api-preflight-20260628T205400Z.json` | UGOS API 仍不能完全替代 SSH/桌面 handoff；临时 token 只允许通过环境变量传入，artifact 不记录 token。Docker 容器列表当前以浏览器 DOM 只读证据补足。 |
| NAS release inputs | 未发现由 NAS collector 返回的正式 `admin-docker-release-inputs/`；NAS 桌面只读观察已确认旧容器和 worker flags，且桌面只读证据转换包 `docs/acceptance/artifacts/admin-docker-nas-desktop-returned-evidence-20260628T210033Z/admin-docker-release-inputs` 已被 precheck/intake 阻断 | 桌面转换包只用于诊断旧栈阻断原因，不能替代正式 NAS collector 返回包；必须通过 NAS desktop/本机 shell/临时 SSH 跑 collector 并把返回包带回 Mac 后才能继续 release review。 |
...
- release readiness refresh：`docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260628T212539Z.json` 已吸收最新 live readonly、NAS access、UGOS preflight 和 Cutter staged proof plan；`release_review_ready=false`、`docker_upload_allowed=false`，仍为 `13` 个 gate、`5` 个通过、`8` 个阻塞。新增 observations 明确 `cutter_staged_plan_ready=true`、candidate `25fe2264de7b391a56e770a8acb6bf40ebec3863`，但最终 `cutter-proof-accepted` 仍阻塞发布，避免把 plan-only 证据误当作 staged proof 通过。
