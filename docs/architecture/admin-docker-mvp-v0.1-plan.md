...
| NAS release inputs | Mac 侧已通过 UGOS browserless 只读采集生成 sanitized returned evidence：`docs/acceptance/artifacts/admin-docker-nas-ugos-returned-evidence-20260628T225531Z/admin-docker-release-inputs`。该证据显示当前旧栈仍为 `latest` 镜像、worker flags 为 `1/1`、缺少 `MIXLAB_ADMIN_DOCKER_MVP_MODE` 与 `MIXLAB_PREPROCESS_LIBRARY_ROOT`、磁盘 `98%`；重新 intake 后 `returned_precheck_passed=true`。最新 intake `docs/acceptance/artifacts/admin-docker-nas-release-inputs-intake-20260629T013145Z.json` 已读取 legacy rollback plan，记录 `legacy_rollback_exception_ready=true`、`accepted=false`，但仍因 `nas-image-proof-accepted`、`admin-worker-proof-accepted`、`nas-disk-proof-accepted` blocked；最新 readiness `docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T013413Z.json` 仍为 `6/13` gate 通过、`docker_upload_allowed=false`。 | UGOS returned evidence 只用于只读诊断和本地门禁输入，不等于正式发布批准；legacy latest rollback exception 只能由 release-manager 角色审评接受，且不能绕过 worker proof、disk proof、Cutter staged proof、显式 push approval 和最终 release review。Staging runbook 会校验手填 current/target/rollback tag 必须与 release-inputs 报告一致。 |
...
- current release inputs：`docs/acceptance/artifacts/admin-docker-release-inputs-20260629T013145Z.json`，仍 `blocked`，`push_execution_allowed=false`，`docker_deploy_allowed=false`；已读取 `docs/acceptance/artifacts/admin-docker-legacy-rollback-plan-20260629T011940Z.json`，并记录 legacy latest rollback exception `ready=true`、`accepted=false`；当前 release-input 阻断为 `legacy-rollback-exception-approved` 与 `nas-image-proof-accepted`。
...
- current staging runbook：`docs/acceptance/artifacts/admin-docker-staging-runbook-20260629T013145Z.json`，`staging_execution_ready=false`，`staging_review_ready=false`，`docker_deploy_allowed=false`；当前真实阻断仍为 current/target/rollback tag、显式 push approval、release inputs、NAS disk proof、parity、admin-worker 与 Cutter staged-candidate proof。
- current release readiness summary：`docs/acceptance/artifacts/admin-docker-release-readiness-summary-20260629T013413Z.json`，`release_review_ready=false`，`docker_upload_allowed=false`；最新汇总仍为 `13` 个 gate、`6` 个通过、`7` 个阻塞，并明确 legacy latest rollback exception `ready=true`、`accepted=false`，需要 release-manager 角色审评；`automation_boundary` 明确本地只读/证据工作可继续，但 NAS runtime 变更、image push、Docker deploy 均不允许。
...
- 2026-06-29 release-input/readiness legacy gate integration：`validate:admin-docker-release-inputs` 与 `intake:admin-docker-nas-release-inputs` 已支持 `MIXLAB_ADMIN_DOCKER_LEGACY_ROLLBACK_PLAN_REPORT`，并要求 `MIXLAB_DOCKER_LEGACY_ROLLBACK_EXCEPTION_APPROVAL=release-manager:legacy-latest-rollback-exception=accepted` 后才把 `latest` 作为 current/rollback 输入；默认未审批状态仍 blocked，且不会改变 `push_execution_allowed=false`、`docker_deploy_allowed=false`。
