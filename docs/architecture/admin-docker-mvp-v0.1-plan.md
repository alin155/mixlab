...
| NAS access preflight | SSH/DSM/Docker TCP/`8080` 关闭；`18080`/`9999` 打开；SMB `/Volumes/MixLab` 已挂载但未发现 compose/.env 或 returned evidence；最新证据 `docs/acceptance/artifacts/admin-docker-nas-access-preflight-20260628T113351Z.json` | 当前只能做只读观测，不能直接从 Mac 收集 NAS Docker release inputs。 |
| NAS release inputs | 未发现 `admin-docker-release-inputs/`、`admin-docker-current.env`、`admin-worker.inspect.json`、`admin-docker-disk-proof.json`；NAS 桌面只读观察已确认旧容器和 worker flags，证据 `docs/acceptance/artifacts/admin-docker-nas-desktop-readonly-20260628T185410Z.json` | 必须通过 NAS desktop/本机 shell/临时 SSH 跑 collector 并把返回包带回 Mac 后才能继续 release review。 |
...
2026-06-28 已完成当前 HEAD 的 GitHub Docker-capable dry-run 证据收口，但仍不代表 NAS 已发布，也不代表可以上传镜像。历史候选 `b062bc387c1fdb2a391320c1c36233b782cb000a` 只保留为旧证据；当前候选已刷新为 `97f2d513a4a27315929b4d964320d4170b4b4631`。

- branch dry-run：GitHub Actions run `28331941051`，ref `codex/windows-first-run-autostart-20260615104835`，head `97f2d513a4a27315929b4d964320d4170b4b4631`，`push_images=false`，workflow success，`current-worktree-candidate-ready`。
- branch dry-run 证据：`docs/acceptance/artifacts/admin-docker-github-run-artifact-20260628T183632Z.json`，对应 artifact readiness `docs/acceptance/artifacts/admin-docker-github-artifact-readiness-20260628T183632Z.json` 为 `candidate-ready`，`docker_deploy_allowed=false`。
- candidate tag：`admin-docker-candidate-97f2d513a4a27315929b4d964320d4170b4b4631`，远端 tag 指向 `97f2d513a4a27315929b4d964320d4170b4b4631`。
- tag dry-run：GitHub Actions run `28332064560`，ref `admin-docker-candidate-97f2d513a4a27315929b4d964320d4170b4b4631`，`push_images=false`，workflow success，`github-run-candidate-ready`。
- tag dry-run 证据：`docs/acceptance/artifacts/admin-docker-github-run-artifact-20260628T184114Z.json`，对应 artifact readiness `docs/acceptance/artifacts/admin-docker-github-artifact-readiness-20260628T184114Z.json` 为 `candidate-ready`，但不作为 staging handoff 通过证据。
- candidate-ref proof：`docs/acceptance/artifacts/admin-docker-candidate-ref-proof-20260628T184119Z.json`，`candidate_ref_proof_accepted=true`，`docker_deploy_allowed=false`。
- pre-staging handoff：`docs/acceptance/artifacts/admin-docker-prestaging-handoff-20260628T184208Z.json`，`ready_to_request_release_inputs=true`，但 `staging_execution_ready=false`；staging 仍被 NAS disk risk、显式 push approval、current/rollback tag、staged live-readonly、admin-worker env proof、Cutter compatibility proof 阻断。
- release inputs：`docs/acceptance/artifacts/admin-docker-release-inputs-20260628T184215Z.json`，因缺 NAS image proof、current/rollback tag 仍 `blocked`，`push_execution_allowed=false`。
- NAS release-inputs handoff：`docs/acceptance/artifacts/admin-docker-nas-release-inputs-handoff-20260628T184221Z.json`，`handoff_package_ready=true`，最新 bundle 指向 `97f2d513a4a27315929b4d964320d4170b4b4631`，包含 `OPERATOR-CHECKLIST.md`，本地 `validate-returned-evidence.sh` 会在 returned evidence precheck 和 intake 后自动刷新 release readiness summary，但不允许 push/deploy。
- NAS handoff portable kit：`docs/acceptance/artifacts/admin-docker-nas-handoff-kit-20260628T184222Z.json`，`kit_ready=true`，便携目录为 `dist/acceptance/admin-docker-nas-handoff-kit`，单文件包为 `dist/acceptance/admin-docker-nas-handoff-kit.tar.gz`，sha256 `c2308dc819b52a5fbf0f83aaf2e175ad7ee4a941620c450ad894df083d008fbf`；包含 `KIT-SELF-CHECK.sh` 与 `KIT-FILES.sha256`，可复制到 NAS desktop/NAS shell host 用于只读收集 `admin-docker-release-inputs/` 返回证据。
...
- live readonly refresh：`docs/acceptance/artifacts/admin-docker-release-live-readonly-20260628T185228Z.json`，GET-only 探测 NAS `http://192.168.1.27:18080`；Admin Web root 可达，但 `/api/admin/auth/status`、`/api/admin/release-gates`、`/api/admin/data-loading/plan` 仍缺失，说明 NAS 仍是旧 Admin API 合约；素材统计保持 ready `10471`、current index `v010471`、total `11394`，queued `904`、index-required `19`，磁盘仍约 `98%` 且 blocked。
