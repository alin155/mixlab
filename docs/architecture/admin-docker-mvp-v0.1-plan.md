...
- post-candidate branch drift boundary：当前分支在 `4cb5b18262e49894d4272b0fc940be6c1d2102b4` 之后已有 release tooling、证据和文档提交；由于 `docker/admin-runtime.Dockerfile` 会 `COPY scripts ./scripts`，不能把当前 HEAD 自动视为已 smoke 的 runtime candidate。除非重新创建候选 tag 并完成 `push_images=false` dry-run / smoke，否则 Docker runtime candidate 仍固定为 `4cb5b18262e49894d4272b0fc940be6c1d2102b4`。
