...
Generated: 2026-06-28T20:37:11.254Z
...
- Env file flags: mvp=missing, preprocess=missing, publish=missing
- Running flags: mvp=missing, preprocess=missing, publish=missing
...
env-file-provided | evidence | blocked | yes | No MIXLAB_ADMIN_WORKER_ENV_FILE was provided. | Provide a sanitized admin-worker.env file from the NAS host containing only the required MVP worker flags.
inspect-json-provided | evidence | blocked | yes | No MIXLAB_ADMIN_WORKER_INSPECT_JSON was provided. | Provide sanitized docker inspect JSON for the running admin-worker container without full Config.Env secrets.
env-file-worker-flags-disabled | worker-flags | blocked | yes | env file: MIXLAB_ADMIN_DOCKER_MVP_MODE=missing, MIXLAB_ENABLE_LIBRARY_PREPROCESS_WORKER=missing, MIXLAB_ENABLE_READY_PUBLISH_WORKER=missing | Admin worker must run in Docker MVP v0.1 mode and both standalone admin-worker flags must be 0 before Docker release gates allow upload or staging.
inspect-worker-flags-disabled | worker-flags | blocked | yes | docker inspect: MIXLAB_ADMIN_DOCKER_MVP_MODE=missing, MIXLAB_ENABLE_LIBRARY_PREPROCESS_WORKER=missing, MIXLAB_ENABLE_READY_PUBLISH_WORKER=missing | Admin worker must run in Docker MVP v0.1 mode and both standalone admin-worker flags must be 0 before Docker release gates allow upload or staging.
...
- JSON: .local-dev/admin-docker-release-gates/admin-worker-env-proof-20260628T203711Z.json
- Markdown: .local-dev/admin-docker-release-gates/admin-worker-env-proof-20260628T203711Z.md
