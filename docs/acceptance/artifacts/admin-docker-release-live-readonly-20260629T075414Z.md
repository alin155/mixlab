...
Generated: 2026-06-29T07:54:14.572Z
...
- Upload blockers: current-admin-api-contract-live, auth-context-live, runtime-path-profile-live, build-version-health, version-health-parity-contract-live, preprocess-disk, disk-space-protection-contract-live, usage-events-tolerance, usage-events-repair-contract-live, processing-recovery, processing-recovery-contract-live, current-index, dashboard-metrics-live, data-loading-contract-live, supervisor-status-live, admin-worker-env-proof-contract-live, admin-worker-live-flags, cutter-compatibility-proof-contract-live, cutter-release-compatibility-live

## Requests

| Probe | Request | HTTP | OK | Duration | Error |
| --- | --- | --- | --- | --- | --- |
admin_web_root | GET / | 200 | yes | 39.7ms | none
auth_status | GET /api/admin/auth/status | 200 | yes | 39.7ms | none
library_status | GET /api/admin/library/status | 401 | no | 5.8ms | login_required
release_gates | GET /api/admin/release-gates | 401 | no | 6.1ms | login_required
dashboard_metrics | GET /api/admin/dashboard/metrics | 401 | no | 6.1ms | login_required
data_loading_plan | GET /api/admin/data-loading/plan | 401 | no | 5.2ms | login_required
preprocess_supervisor_status | GET /api/admin/preprocess/supervisor/status | 401 | no | 5.4ms | login_required
...
admin-api-proxy-live | Admin API proxy reachable through Admin Web | live-admin | pass | no | At least one Admin API JSON endpoint responded; auth=HTTP 200, ok, library=HTTP 401, login_required 请先登录管理端, supervisor=HTTP 401, login_required 请先登录管理端 | n/a
current-admin-api-contract-live | Current Admin Architecture v1 API contract is live | live-admin | blocked | yes | Required current endpoints are missing or blocked: auth=HTTP 200, ok, release_gates=HTTP 401, login_required 请先登录管理端, data_loading_plan=HTTP 401, login_required 请先登录管理端 | Deploy or point to an Admin API image exposing /api/admin/auth/status, /api/admin/release-gates, and /api/admin/data-loading/plan.
auth-context-live | Protected live gate endpoints are readable | live-admin | blocked | yes | auth_mode=password, authenticated=false; protected endpoints may require MIXLAB_ADMIN_DOCKER_LIVE_SESSION_TOKEN. | Provide a reviewed Admin session token only through MIXLAB_ADMIN_DOCKER_LIVE_SESSION_TOKEN; do not write it into docs.
...
dashboard-metrics-live | Dashboard metrics read does not fail live | live-admin | blocked | yes | Dashboard metrics not proven: HTTP 401, login_required 请先登录管理端 | Run GET /api/admin/dashboard/metrics successfully against the NAS Docker target.
data-loading-contract-live | Data-loading contract is readable live | live-admin | blocked | yes | Data-loading plan not proven: HTTP 401, login_required 请先登录管理端 | Run GET /api/admin/data-loading/plan successfully against the NAS Docker target.
supervisor-status-live | Supervisor status is readable without starting workers | live-admin | blocked | yes | Supervisor status not proven: HTTP 401, login_required 请先登录管理端 | Run GET /api/admin/preprocess/supervisor/status successfully; do not call start/stop.
...
- JSON: docs/acceptance/artifacts/admin-docker-release-live-readonly-20260629T075414Z.json
- Markdown: docs/acceptance/artifacts/admin-docker-release-live-readonly-20260629T075414Z.md
