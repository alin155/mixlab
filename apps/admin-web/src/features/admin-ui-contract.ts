...
export const ADMIN_UI_PAGES: Record<AdminRoute, AdminPageContract> = {
  dashboard: {
    route: "dashboard",
    label: "首页",
    goal: "让小白用户一眼看懂素材生产是否正常",
    primaryQuestion: "系统是否正常，素材是否在继续处理，剪辑端是否可用？",
    composition: {
      role: "overview",
  ...
  "source-videos": {
    ...
    controls: [
      ...
      { route: "source-videos", label: "上线到剪辑端", state: "m9b-api", reason: "在当前素材详情中上线已处理待上线素材。" },
  ...
  "preprocess-jobs": {
    ...
    label: "素材处理",
    goal: "让小白用户用一个页面继续处理素材并上线给剪辑端",
    primaryQuestion: "还有多少素材要处理，系统是否在处理，已处理素材是否能上线？",
    composition: {
    ...
    controls: [
      { route: "preprocess-jobs", label: "启动预处理", state: "m9b-api", reason: "发现素材、加入队列、预处理，并在安全时上线。" },
  ...
  "index-publish": {
    ...
    controls: [
      ...
      { route: "index-publish", label: "上线全部已处理素材", state: "m9b-api", reason: "批量上线所有已处理待上线素材。" },
