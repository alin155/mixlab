...
async function readStore(libraryRoot: string): Promise<AdminUserStore> {
  ...
  try {
    ...
    throw new Error("管理员用户存储文件格式错误", { cause: error });
  }
...
async function writeStore(libraryRoot: string, store: AdminUserStore): Promise<void> {
  ...
  const tempPath = path.join(targetDir, `.users.${process.pid}.${Date.now()}.${randomUUID()}.tmp`);
...
export async function validateAdminSession(
  libraryRoot: string,
  input: { session_token: string; now: string }
